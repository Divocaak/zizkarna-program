...
const eventStartShift = 0.5;
const eventEndShift = 0.4;
...
export async function POST({ request }) {
	...
	data.events.forEach((eventData, index) => {
		...
		const textPart =
			!twoSections || (index < finalMiddleIndex && twoSections)
				? textParts.first
				: textParts.second;

		const isLast =
			index === data.events.length - 1 || (twoSections && index === finalMiddleIndex - 1);
		textPart.pushRow({
	...
	if (twoSections) {
		const contentLenPerSection =
			(outputMediumOrVidLength -
				firstInDuration -
				textParts.second.getRowsCount() * (eventEndShift + eventStartShift)) /
			2;
		const firstGetOutStartNew =
			textParts.first.getInStart() + firstInDuration + contentLenPerSection;
...
const videoElements = ({
	...
	const eventFadeInDelay = 0.25;
	const eventFadeOutDelay = 0.1;
	...
	const toRet = [
		new ImageVideoElement({
			...
			posY: padding.getTop() - logoH * 0.25,
			wPx: logoW,
			hPx: logoH
		}),
