...
export class MonthlyOverviewEventRow extends VideoElement {
	...
	#calculateTimes(inputInStart, inputOutStart) {
		...
		var lineTimes = null;
		if (!this.isFirst)
			lineTimes = createTimes(inputInStart, inputOutStart, lineDelay, lineAnimationLength);
	 ...
	 */
	createVideoObjects({ parentId, timeInStart = 0, timeOutStart = 0, dynamicStyles, outputWidth }) {
		...
		if (!this.isLast)
			labelStyles += `padding-bottom: ${dynamicStyles.getPadding('label')}px !important;`;

		this.#dateVideoObject = new TextVideoElement({
