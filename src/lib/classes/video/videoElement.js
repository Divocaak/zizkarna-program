...
export class VideoElement {
	 ...
	 */
	constructor({ id, content, posX = 0, posY = 0, easing = null }) {
		this.id = id;
	 ...
	 */
	getStyles({ time, additionalStyles = '' }) {
		return `#${this.getId()}{
            position: absolute;
	...
	#getXInTime(time) {
		return Array.isArray(this.posX)
			? this.#interpolateKeyframes({ keyframes: this.posX, time: time, easing: this.easing })
			: this.posX;
	}

	#getYInTime(time) {
		return Array.isArray(this.posY)
			? this.#interpolateKeyframes({ keyframes: this.posY, time: time, easing: this.easing })
			: this.posY;
	}

	getWidthInTime(time) {
		return Array.isArray(this.width)
			? this.#interpolateKeyframes({ keyframes: this.width, time: time, easing: this.easing })
			: this.width;
	}
	...
	#easeInOutBack(x) {
		const c1 = 1.70158 * 0.75;
		const c2 = c1 * 1.525 * 0.75;
